function updateNear(params){
	var link =  $('#events-in-range a').attr('href')
	link+="?lat="+params.lat+"&lng="+params.lng;
	$('#events-in-range a').attr('href',link);

	AjaxClient.eventsNear(params,function(data){
		$('#events-number').html(data.length);
		$('#events-number2').show();
		$('#events-in-range .red-arrow img').attr('src','/img/white-arrow.png');
		$('#events-in-range').addClass('has-near-events');
	});
}
