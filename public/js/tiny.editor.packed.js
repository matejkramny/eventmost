...
TINY.editor=function(){var c=[],offset=-30;
	...
	function edit(n,obj){
		...
		for(i;i<l;i++){
			...
			for(x;x<fl;x++){
				var font=fonts[x];
				sel.options[x+1]=new Option(font,font)}h.appendChild(sel)}
				...
				for(x;x<sl;x++){var size=sizes[x];sel.options[x]=new Option(size,size)}h.appendChild(sel)}
