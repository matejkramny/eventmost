...
TINY.editor=function(){var c=[],offset=-30;
	...
	c['outdent']=[12,'Outdent','a','outdent'];c['indent']=[13,'Indent','a','indent'];
	...
	function edit(n,obj){
		...
		for(i;i<l;i++){
			...
			{var d=document.createElement('div');d.className=obj.dividerclass||'tinyeditor-divider';h.appendChild(d)}
				...
				{var sel=document.createElement('select'),fonts=obj.fonts||['Verdana','Arial','Georgia','Raleway'],fl=fonts.length,x=0;
			sel.className='tinyeditor-font';sel.onchange=new Function(this.n+'.ddaction(this, "fontname")');
			sel.options[0]=new Option('Font Family','');
			for(x;x<fl;x++){
				var font=fonts[x];
				sel.options[x+1]=new Option(font,font)}
				var selDiv = document.createElement('div');
                selDiv.className = 'tinyeditor-font-over';
                selDiv.appendChild(sel);
                h.appendChild(selDiv);
				//h.appendChild(sel)
			}
				...
				for(x;x<sl;x++){var size=sizes[x];sel.options[x]=new Option(size,size)}
				var selDiv = document.createElement('div');
                selDiv.className = 'tinyeditor-font-over2';
                selDiv.appendChild(sel);
                h.appendChild(selDiv);
            }
					

					else if(id=='style'){var sel=document.createElement('select'),styles=obj.styles||
						[['Style',''],['Paragraph','<p>'],['Header 1','<h1>'],['Header 2','<h2>'],['Header 3','<h3>'],['Header 4','<h4>'],
					['Header 5','<h5>'],['Header 6','<h6>']],
					sl=styles.length,x=0;sel.className='tinyeditor-style';sel.onchange=new Function(this.n+'.ddaction(this, "formatblock")');
					for(x;x<sl;x++){var style=styles[x];sel.options[x]=new Option(style[0],style[1])}h.appendChild(sel)}
						else if(c[id]){var div=document.createElement('div'),x=c[id],func=x[2],ex,pos=x[0]*offset;
					div.className=obj.controlclass;div.unselectable='on';div.style.backgroundPosition='0px '+pos+'px';div.title=x[1];
